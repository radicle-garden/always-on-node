...
resource "scaleway_instance_ip" "aon_public_ip_staging" {
  project_id = var.scaleway_project_id
  zone = var.scaleway_zone
  tags = ["radiclegarden", "always-on-node"]
}

resource "scaleway_block_volume" "aon_nodes_data_staging" {
  name = "aon_nodes_data"
...
resource "scaleway_block_volume" "aon_logs_staging" {
  name = "node_logs"
...
resource "scaleway_instance_server" "aon_staging" {

  project_id = var.scaleway_project_id
  name       = "always-on-node_staging"
  type       = "BASIC2-A2C-4G"
  image      = data.scaleway_instance_image.packer_vm_staging.image_id
  zone       = var.scaleway_zone

  tags = ["radiclegarden", "always-on-node"]

  ip_id = scaleway_instance_ip.aon_public_ip_staging.id
  ...
  additional_volume_ids = [
    scaleway_block_volume.aon_logs_staging.id,
    scaleway_block_volume.aon_nodes_data_staging.id,
  ]
...
resource "cloudflare_dns_record" "nodes_record_staging" {
  provider = cloudflare
...
data "scaleway_secret" "pg_user_staging" {
  name = "PG_USER_STAGING"
}
data "scaleway_secret_version" "pg_user_staging" {
  secret_id = data.scaleway_secret.pg_user_staging.id
  revision  = "latest"
}
data "scaleway_secret" "pg_pass_staging" {
  name = "PG_PASS_STAGING"
}
data "scaleway_secret_version" "pg_pass_staging" {
  secret_id = data.scaleway_secret.pg_pass_staging.id
  revision  = "latest"
}

resource "scaleway_rdb_instance" "staging" {
  name           = "garden-staging-db-server"
  node_type      = "DB-DEV-S"
  engine         = "PostgreSQL-17"
  is_ha_cluster  = false
  disable_backup = false
  user_name      = base64decode(data.scaleway_secret_version.pg_user_staging.data)
  password       = base64decode(data.scaleway_secret_version.pg_pass_staging.data)
...
resource "scaleway_rdb_acl" "staging" {
